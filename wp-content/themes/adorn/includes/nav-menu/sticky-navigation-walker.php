...
if (!class_exists('AdornEdgeStickyNavigationWalker')) {
	class AdornEdgeStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $featured_icon .= '<i class="edge-menu-featured-icon '.$item->featured_icon .'"></i>';
                }
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i aria-hidden="true" class="edge-menu-arrow arrow_triangle-down"></i>';
				}
