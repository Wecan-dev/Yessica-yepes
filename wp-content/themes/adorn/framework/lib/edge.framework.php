...
class AdornEdgeFramework {
    ...
    private function __construct() {
        $this->edgeOptions = AdornEdgeOptions::get_instance();
        $this->edgeMetaBoxes = AdornEdgeMetaBoxes::get_instance();
    }
	...
	public function setSkin(AdornEdgeSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AdornEdgeSkinManager
 *
...
/**
 * Class AdornEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AdornEdgeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(adorn_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
		...
		return EDGE_ASSETS_ROOT.'/img/logo.ico';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgeSCIcon = '<?php echo adorn_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
			window.edgeSCLabel = '<?php echo esc_html(ucfirst($this->skinName)); ?> Shortcodes';
		</script>
	<?php }
...
class AdornEdgeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $adorn_options)) {
            return $adorn_options[$key];
...
/*
   Class: AdornEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class AdornEdgeAdminPage implements iAdornEdgeLayoutNode {
    ...
    function __construct($slug="", $title_label="", $icon = "") {
        $this->layout = array();
        $this->factory = new AdornEdgeFieldFactory();
        $this->slug = $slug;
    ...
    public function renderChild(iAdornEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: AdornEdgeMetaBoxes
...
/*
   Class: AdornEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class AdornEdgeMetaBox implements iAdornEdgeLayoutNode {
	...
	public function setTitle($label) {
		...
		return $adorn_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if (!function_exists( 'adorn_edge_init_framework_variable')) {
    function adorn_edge_init_framework_variable() {
        global $adorn_Framework;

        $adorn_Framework = AdornEdgeFramework::get_instance();
        $edgeSkinManager = new AdornEdgeSkinManager();
        $adorn_Framework->setSkin($edgeSkinManager->getSkin());
    }

    add_action('adorn_edge_before_options_map', 'adorn_edge_init_framework_variable');
}
?>