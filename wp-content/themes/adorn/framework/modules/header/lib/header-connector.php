<?php
namespace Adorn\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('adorn_edge_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('adorn_edge_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('adorn_edge_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('adorn_edge_after_header_connect');
    }
     ...
     *
     * Hooks to adorn_edge_title_content_padding filter
     *
     * @return int
     */
