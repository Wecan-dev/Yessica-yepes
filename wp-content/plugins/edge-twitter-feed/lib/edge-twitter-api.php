...
include_once 'edge-twitter-helper.php';

/**
 * Class EdgeTwitterApi
 */
class EdgeTwitterApi {
    /**
    ...
    const REQUEST_TOKEN_FIELD = 'edge_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'edge_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'edge_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'edge_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'edge_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'edge_twitter_authorize_verifier';
    const USER_ID_FIELD = 'edge_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'edge_twitter_screen_name';
    ...
    public function __construct() {
        ...
        $this->helper = new EdgeTwitterHelper();
    }
    /**
     ...
     *
     * @see EdgeTwitterApi::generateSignatureBase()
     */
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'edge-twitter-feed');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\'t be generated', 'edge-twitter-feed');
                    } else {
                        $responseObj->status = true;
                        $responseObj->message = esc_html__('Ok', 'edge-twitter-feed');
                    }
                } else {
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'edge-twitter-feed');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'edge-twitter-feed');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtained', 'edge-twitter-feed');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'edge-twitter-feed');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seems like you haven\'t connected with your Twitter account', 'edge-twitter-feed');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retrieve content from database', 'edge-twitter-feed');
            }
        }
