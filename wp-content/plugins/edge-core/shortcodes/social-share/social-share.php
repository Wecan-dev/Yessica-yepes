...
class SocialShare implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name' => esc_html__('Edge Social Share', 'edge-core'),
            'base' => $this->getBase(),
            ...
            'params' => array(
                ...
                array(
                    ...
                    'heading' => esc_html__('Icons Type', 'edge-core'),
                    'value' => array(
                        esc_html__('Font Awesome', 'edge-core') => 'font-awesome',
                        esc_html__('Font Elegant', 'edge-core') => 'font-elegant'
                    )
                ),
                array(
                    'type' => 'textfield',
                    'param_name' => 'title',
                    'heading' => esc_html__('Social Share Title', 'edge-core'),
    ...
    public function render($atts, $content = null) {
        ...
        $params['enable_social_share'] = (adorn_edge_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

        //Is social share enabled for post type
        $post_type = get_post_type();
        $params['enabled'] = (adorn_edge_options()->getOptionValue('enable_social_share_on_' . $post_type) == 'yes') ? true : false;
        ...
        if ($params['enable_social_share']) {
            if ($params['enabled']) {
                $html .= edge_core_get_shortcode_module_template_part('templates/' . $params['type'], 'social-share', '', $params);
            }
        }
    ...
    private function getSocialNetworksParams($params) {
        ...
        foreach ($this->socialNetworks as $net) {
            ...
            $html = '';
            if (adorn_edge_options()->getOptionValue('enable_' . $net . '_share') == 'yes') {
                ...
                $params['custom_icon'] = (adorn_edge_options()->getOptionValue($net . '_icon')) ? adorn_edge_options()->getOptionValue($net . '_icon') : '';
                $html = edge_core_get_shortcode_module_template_part('templates/parts/network', 'social-share', '', $params);
            }
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (adorn_edge_options()->getOptionValue('twitter_via') !== '') ? ' via ' . adorn_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(adorn_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(adorn_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . adorn_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
