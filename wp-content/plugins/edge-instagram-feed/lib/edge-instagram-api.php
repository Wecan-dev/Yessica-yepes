...
include 'edge-instagram-helper.php';

/**
 * Class EdgeInstagramApi
 */
class EdgeInstagramApi {
    /**
    ...
    private $apiURL;
    /**
     * @var EdgeInstagramApi instance of current class
     */
    private static $instance;

    /**
     * Helper object
     * @var EdgeInstagramHelper
     */
    private $helper;

    const CODE_OPTION_NAME = 'edge_instagram_code';
    const USER_ID_OPTION_NAME = 'edge_instagram_user_id';
    const ACCESS_TOKEN_OPTION_NAME = 'edge_instagram_access_token';
    const CODE_REDIRECT_URI_OPTION_NAME = 'edge_instagram_code_redirect_uri';
    ...
    private function __construct() {
	    ...
	    $this->helper = new EdgeInstagramHelper();
    }
     ...
     * @return string
     *
     * @see EdgeInstagramApi::buildCurrentPageURI()
     */
     ...
     *
     * @see EdgeInstagramApi::fetchData()
     */
     ...
     * @return stdClass
     *
     * @see EdgeInstagramApi::getAccessToken()
     */
    private function fetchData($limit = '', $tag = '', $transient = array('use_transient' => false)) {
        ...
        $returnObject->message = esc_html__('Success', 'edge-instagram-feed');
    ...
    public function getAccessToken() {
        ...
        if(empty($this->code)) {
            $returnObject->status = 'error';
            $returnObject->message = esc_html__('User hasn\'t connected with Instagram', 'edge-instagram-feed');
            ...
            if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\'t connect with Instagram API', 'edge-instagram-feed');
            ...
            if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
                ...
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\'t get Instagram access token', 'edge-instagram-feed');
        ...
        //prepare return object and return it
        $returnObject->status = 'ok';
        $returnObject->message = esc_html__('Stored access token', 'edge-instagram-feed');
